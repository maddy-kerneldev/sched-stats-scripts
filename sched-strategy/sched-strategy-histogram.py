...
# Script Uages:
#
#   #sudo perf script -i <path to perf.data file> -s ./sched-strategy-histogram.py
#
...
#disct = {<pid>: {<tid>:{'name':"", 'cpu':0, 'disp':0, 'affin':0, 'sc':0, 'bc':0, 'oc':0,'nsec':0, 'hist':[0,0,0,0,0,0]}}}
#disct = {<cpu>: {<pid>: {<tid>:{ 'nsec':0}}}}
pid_dis = {}
cpu_dis = {}
lnsecs = 0

def trace_end():
    print("%15s %9s %9s %7s %7s %7s %7s %7s %7s %7s %7s %7s | %7s %5s %5s %3s %7s %7s" % ("Process", "tid", "pid", "#Disp", "#affin", "%affin", "#sc", "%sc", "#bc", "%bc", "#oc", "%oc", "0-100", '101-500', '501-999', '2', '3-4', '>4'))
    print("%15s %9s %9s %7s %7s %7s %7s %7s %7s %7s %7s %7s | %7s %5s %5s %7s %7s %7s" % ("", "", "", "", "", "", "", "", "", "", "", "", "us", 'us', 'us', 'ms', 'ms', 'ms'))
    print("--------------------------------------------------------------------------------------------------------------------------------------------------------------")
    for pid, tid_dis in sorted(pid_dis.items()):
        for tid in tid_dis:
            print('%15s %9s %9s %7d %7d %7.2f %7d %7.2f %7d %7.2f %7d %7.2f | %7s %5s %5s %7s %7s %7s' % (
                tid_dis[tid]['name'],
                ...
                tid_dis[tid]['hist'][0], tid_dis[tid]['hist'][1],
                tid_dis[tid]['hist'][2],tid_dis[tid]['hist'][3],
                tid_dis[tid]['hist'][4],tid_dis[tid]['hist'][5]))
...
def sched__sched_switch(event_name, context, common_cpu,
    ...
    if (pid in pid_dis):
        if (tid in pid_dis[pid]):
            ...
            pid_dis[pid][tid]['nsec'] = common_nsecs
        else:
            pid_dis[pid][tid] = {
